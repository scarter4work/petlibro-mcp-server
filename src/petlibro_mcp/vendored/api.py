...
class PetLibroSession:
    ...
    def __init__(self, base_url: str, email: str, password: str, region: str, websession: ClientSession | None = None, token: str | None = None, time_zone: str | None = None):
    ...
    async def _ensure_websession(self) -> ClientSession:
        """Lazily create an aiohttp session if one wasn't injected."""
    ...
    async def close(self) -> None:
        ...
        if self._owns_websession and self.websession is not None:
    ...
    async def request(self, method: str, url: str, **kwargs: Any) -> JSON:
        ...
        # Send the request
        websession = await self._ensure_websession()
        async with websession.request(method, joined_url, **kwargs) as resp:
    ...
    async def re_login(self) -> str:
        ...
        try:
            ...
            raise PetLibroInvalidAuth(f"Client error during re-login: {e}") from e
            ...
            raise PetLibroInvalidAuth(f"Unexpected error during re-login: {e}") from e
...
class PetLibroAPI:
    ...
    def __init__(
        ...
        `websession` is optional: when omitted, `PetLibroSession` lazily creates
        its own `aiohttp.ClientSession` on first use (see `PetLibroSession.close`).
        """
        self.time_zone = time_zone or "America/Chicago"
        self.session = PetLibroSession(self.API_URLS[region], email, password, region, websession, token, self.time_zone)
        self.region = region
